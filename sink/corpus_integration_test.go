...
func TestClickHouseLinkStateFromCorpus(t *testing.T) {
	...
	// Queried here against the real, replayed table.
	var routerIDv4 string
	if err := ch.conn.QueryRow(ctx, qualify(ch,
		"SELECT router_id_v4 FROM vantage.ls_nodes WHERE name = 'xr-rr1' LIMIT 1")).
		Scan(&routerIDv4); err != nil {
		t.Fatalf("query ls_nodes (router_id_v4): %v", err)
	}
	if routerIDv4 != "10.255.0.1" {
		t.Errorf("router_id_v4 = %q, want 10.255.0.1", routerIDv4)
	}
}
