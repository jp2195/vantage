.PHONY: help check chart-deps dev-api generate test test-nodb build fuzz images push-images ui ui-gen ui-test ui-fixtures cla-test standalone-check sync-helm-schema fetch-asnames restore-archive-backup
...
cla-test: ## Test the CLA check behind .github/workflows/cla.yml (Node only, no install)
	node --test .github/cla/*.test.js
