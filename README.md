...
decoded into typed rows — not kept as opaque bytes — with per-vendor quirk
handling and a parse-flag field on every row recording what the decoder
could not type. RFC 8671 adj-RIB-out is part of peer identity rather than a
flag on a shared peer, so pre-policy in, post-policy in, pre-policy out and
post-policy out are four separate answers rather than one blurred together.
...
The design principle that shows up on every screen: **an answer states its
own limits.** A paginated walk says it is a smear over a live table rather
than a snapshot. A peer still sending its initial dump is marked
`provisional` rather than counted as settled. A collector that stopped
hearing from a session reports `collector lost view` — its own blindness —
which is a different fact from the router reporting a BGP session `down`,
and no screen merges the two. An empty chart says it is a bucket with no
rows rather than rendering as a gap.
...
both collectors, with state (`up`, `dumping`, `view lost`), route count,
`Up since`, an archived-rows-per-second rate and a per-peer `Changes`
sparkline. `Up since` is scoped honestly: it shows only while that session
is up, because a session the collector has stopped hearing from would
otherwise report the same value forever and read as a liveness claim.
...
Each collector's own `/status`: peers up, liveness, uptime, publish errors
and rejects, rows archived per minute, and the routers it is hearing from
with whatever `sysDescr` each one sent. A collector that is not answering is
reported as not answering, rather than inferred from an absence of rows.
...
not move those bindings. The two collectors' BMP ports (`11019`, `11119`)
are the exception, bound to `0.0.0.0`: a BMP listener that only accepts
loopback connections can never accept a router. The dev stack's Grafana runs with anonymous access
against a ClickHouse datasource, so anyone who can reach that port can run
arbitrary SQL.
...
- **NATS** is a conditional subchart, or point `nats.externalURL` at your
  own.
...
chart pulls matching images with no overrides. Until a release exists,
build your own with `make push-images REGISTRY=<your-registry>` and point
`images.*.repository` and `images.*.tag` at them.
...
`scripts/citation-allow.txt` (see CONTRIBUTING.md).

## License

Copyright © 2026 jp2195.

vantage is licensed under the **GNU General Public License v3** — see
[`LICENSE`](LICENSE). Contributions are accepted under a Contributor License
Agreement ([`CLA.md`](CLA.md)); [`CONTRIBUTING.md`](CONTRIBUTING.md) explains
why, and what this project expects of a change.
